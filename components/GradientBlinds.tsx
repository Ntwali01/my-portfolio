...
export default function GradientBlinds({
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (t - lastRenderTimeRef.current < 33) return;
      ...
      if (mouseDampening > 0) {
        ...
        uniforms.iMouse.value[0] += (tx - uniforms.iMouse.value[0]) * factor;
        uniforms.iMouse.value[1] += (ty - uniforms.iMouse.value[1]) * factor;
      } else {
        lastTimeRef.current = t;
      }
