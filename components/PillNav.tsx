...
export default function PillNav({
  ...
  baseColor = '#111111',
  pillColor = '#1a1a1a',
  hoveredPillTextColor = '#00d4c8',
  pillTextColor,
  onMobileMenuClick,
  initialLoadAnimation = true,
}: PillNavProps) {
  const resolvedPillTextColor = pillTextColor ?? '#e5e5e5';
  ...
  const handleEnter = (i: number) => {
    ...
    activeTweenRefs.current[i] = tl.tweenTo(tl.duration(), { duration: 0.45, ease: 'power2.out', overwrite: 'auto' }) as gsap.core.Tween;
  };
  ...
  const handleLeave = (i: number) => {
    ...
    if (!tl) return;
    activeTweenRefs.current[i]?.kill();
    activeTweenRefs.current[i] = tl.tweenTo(0, { duration: 0.7, ease: 'power1.inOut', overwrite: 'auto' }) as gsap.core.Tween;
  };
      ...
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        ...
        >
          {logo ? (
            ...
            <span className="pill-logo-text">CA</span>
          )}
        </Link>
